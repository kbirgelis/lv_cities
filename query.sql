--KRISTERS KEVINS BIRGELIS

CREATE TABLE cities (
   `name`     VARCHAR(100) NOT NULL PRIMARY KEY,
   `year`  INTEGER  NOT NULL,
   `population` INTEGER  NOT NULL,
   `novads` VARCHAR(100) NOT NULL
);
INSERT INTO cities (`name`,`year`,`population`,`novads`) VALUES
...
SELECT name, novads FROM cities;
...
SELECT DISTINCT novads FROM cities;

DELETE FROM cities WHERE name='Durbe';

SELECT novads, SUM(population) AS total_population
FROM cities
GROUP BY novads
